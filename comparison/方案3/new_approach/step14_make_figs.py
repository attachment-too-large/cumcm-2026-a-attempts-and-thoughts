# -*- coding: utf-8 -*-
"""生成 PDF 用的插图（中文字体：Microsoft YaHei）。"""
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
dt_lad = np.array([40., 20., 10., 5., 2., 1., 0.5, 0.25])
td_dt = np.array([205508.7, 205538.8, 205553.9, 205561.5, 205566.0,
                  205567.5, 205568.15, 205568.48])
N_lad = np.array([800., 1600., 3200., 6400.])
td_N = np.array([205550.98, 205567.49, 205572.43, 205573.77])
...
fig, ax = plt.subplots(1, 2, figsize=(9.4, 3.5))
ax[0].semilogx(dt_lad, td_dt - LIM, "o-", lw=1.6, ms=5,
               label="paper 格式 FV，N=1600")
ref = td_dt[-1] - LIM
ax[0].semilogx(dt_lad, (td_dt[5] - LIM) * (dt_lad / 1.0), "k--", lw=1.1,
               label="斜率 1（一阶）参考线")
ax[0].set_xlabel(r"时间步 $\Delta t$ / s")
ax[0].set_ylabel(r"$t_{dry}$ 与收敛值之差 / s")
ax[0].set_title(r"(a) 时间步收敛：$\Delta t$ 一阶")
ax[0].grid(alpha=.3, which="both"); ax[0].legend(fontsize=8.5); ax[0].invert_xaxis()

ax[1].loglog(N_lad, LIM - td_N, "s-", lw=1.6, ms=5,
             label="paper 格式 FV，$\\Delta t$=1 s")
ax[1].loglog(N_sp, LIM - td_sp, "^-", lw=1.6, ms=5, label="本文谱配置法")
ax[1].loglog(N_lad, (LIM - td_N[-1]) * (N_lad / 6400.0) ** -2, "k--", lw=1.1,
             label="斜率 $-2$（二阶）参考线")
ax[1].set_xlabel("节点数 N"); ax[1].set_ylabel(r"$t_{dry}$ 与收敛值之差 / s")
ax[1].set_title("(b) 空间收敛：谱解远快于二阶")
ax[1].grid(alpha=.3, which="both"); ax[1].legend(fontsize=8.5)
fig.tight_layout(); fig.savefig(OUT + r"\fig3_tdry_conv.png"); plt.close(fig)

print("tdry3 = %.3f" % tdry3)
print("figs done")
