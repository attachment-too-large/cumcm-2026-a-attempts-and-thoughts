# -*- coding: utf-8 -*-
"""复现 paper 自己的格式（节点中心有限体积 + theta 法），用于定位差异来源。
...
时间: theta 法, theta=0.5(CN), 前 4 步 theta=1 (Rannacher)。
"""
import numpy as np
    ...
    while t < hi - 1e-9:
        ...
        for (t_lim, d) in dt_plan:
            if t < t_lim:
                dt = d
                break
        R = float(cm.R_of(t + dt)) if shrink else cm.R0   # paper: R(t_new)
        Cair = float(cm.C_air(t + 0.5 * dt))              # paper: t_mid
