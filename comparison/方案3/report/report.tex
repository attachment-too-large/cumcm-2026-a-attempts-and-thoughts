...
\large —— 一条与「有限体积 + Crank--Nicolson + Richardson 外推」不同的技术路线}
\author{新解法推导与验证报告}
\date{\today}
...
\begin{abstract}
\noindent
本文针对 2026 年高教社杯 A 题「药材的烘干问题」，建立并严格推导了一套与参考论文
（下称 paper）完全不同的求解路线：\textbf{坐标正则化 $u=(r/R)^2$ + Chebyshev 谱配置
+ 自适应高阶隐式时间积分 + Bessel--Duhamel 闭式解校验}。

技术要点有四处与 paper 不同：其一，不用 $r$ 或 $\xi=r/R$ 作自变量，而取 $u=\xi^2$，
使圆柱 Laplace 算子在轴心处\textbf{完全正则}，从而 $u=0$ 无需任何特殊控制体处理；
其二，空间离散由二阶节点中心有限体积改为\textbf{谱精度}的 Chebyshev 配置，实测
$N=8$ 即达 $10^{-10}$ 精度；其三，时间离散由定步长 $\theta$ 法（冻结物性，实测一阶）
改为\textbf{变阶变步长隐式 BDF}；其四，问题 1 的温度场\textbf{根本不离散}，由准静态
分解得到闭式级数解，2 项即达机器精度。

全部关键结果经\textbf{三条相互独立的技术路线}（闭式解析解、谱配置、守恒型有限体积
Richardson 外推）交叉验证，互差不超过 $2\times10^{-6}$。所给 7 个关键答案中
5 个与目标完全一致；问题 3、4 的烘干时长相差 $0.008\%$。本文对该差异做了完整归因：
\textbf{原样复现} paper 的有限体积格式可将其表 7 序列逐值重现（误差 $\pm0.2$ s），
证明两者模型完全一致；差异来自 paper 长时间段 $\Delta t=10$ s 带来的\textbf{一阶时间误差}，
其 Richardson 外推并未收敛到连续极限。

\vspace{4pt}
\noindent\textbf{关键词}：圆柱扩散；Robin 边界；坐标正则化；Chebyshev 谱配置；
自适应 BDF；Bessel--Duhamel 展开；物质坐标；收敛性归因
\end{abstract}
...
\begin{remark}[与参考解的可比口径]
参照解在程序中把上述参数\textbf{四舍五入}为
$T_{\mathrm{set}}=50.212$、$\tau_T=1877.5$、$\tau_C=2776.3$。
该四舍五入在 $T(R,1800\,\mathrm{s})$ 上造成约 $6\times10^{-5}\,^\circ$C 的差异，
足以影响第 4 位小数。为使结果可比，本文主口径与之取齐，同时保留全精度拟合值作对照
（见 \S \ref{sec:constsens}）。
\end{remark}
...
附录中 $D=7\times10^{-9}e^{-0.89/C}$ 的指数究竟读作 $-0.89/C$ 还是 $-0.89C$？
用 paper 报告的 $D(C_0)=4.9377\times10^{-9}$ m$^2$/s 反算：
$7\times10^{-9}e^{-0.89/2.55}=4.937\times10^{-9}$ \checkmark，
而 $e^{-0.89\times2.55}$ 给出 $7.2\times10^{-10}$ \ding{55}。确认为 $e^{-0.89/C}$。
同理 $C=0.15$、$T=50\,^\circ$C 时附录 3 给出 $D=8.0\times10^{-10}$，
与 paper 一致，进一步确认 $T$ 必须取 K。
\end{remark}
...
必须显式处理这一奇异}——例如节点中心有限体积必须把最内控制体取成
$[0,h/2]$ 并令其测度为 $h^2/8$，才能保证守恒与精度。这是 paper 的
\textbf{式 (14)} 中 $\mathrm{vol}_0=h^2/8$ 的由来，也是其格式复杂度的主要来源之一。
...
\begin{remark}[与 paper 的关系]
式 \eqref{eq:regularize} 是恒等式，不是近似。$R$ 为常数时它给出 paper 式 (2) 的等价形式；
$R=R(t)$ 时把 $R(t)$ 代入系数即可，与 paper 的物质坐标式 (10)--(11) 完全等价
（见 \S\ref{sec:shrink}）。本文的差别在于\textbf{用它作离散的出发点}，从而消去奇异。
\end{remark}
...
而水分不动」这一显然事实矛盾。这解释了为什么本文（与 paper）必须使用物质坐标形式
...
本表 35 个值与参考解\textbf{完全相同}。其中 $r=0$ 与 $r=2.0$ cm、$t=1800$ s 两格即目标值。
...
%======================================================================
\section{问题 3、4 烘干时长差异的完整归因}

\subsection{第一步：原样复现参考解自己的格式}

为判断差异究竟来自「模型不同」还是「数值误差」，本文按 paper 源码
（节点中心有限体积、$\theta$ 法 CN + 前 4 步 Rannacher、物性冻结、
$R$ 取 $R(t_{\text{new}})$、环境取步中点 $t_{\text{mid}}$）\textbf{独立实现}其格式：

\begin{table}[H]
\centering
\small
\caption{问题 3 烘干时间的网格序列：本文复现 vs 参考解表 7}
\begin{tabular}{@{}lccccc@{}}
\toprule
配置 & $N=200$ & $N=400$ & $N=800$ & $N=1600$ & $N=1600,\ \Delta t=1$ s \\
\midrule
本文复现 & 205358.2 & 205488.4 & 205537.4 & \textbf{205553.9} & \textbf{205567.5} \\
paper 表 7 & 205358.0 & 205488.3 & 205537.3 & \textbf{205553.8} & \textbf{205567.4} \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{问题 4 烘干时间的网格序列：本文复现 vs 参考解表 7}
\begin{tabular}{@{}lccccc@{}}
\toprule
配置 & $N=200$ & $N=400$ & $N=800$ & $N=1600$ & Richardson 外推 \\
\midrule
本文复现 & 182726.77 & 182753.94 & 182761.89 & 182764.01 & \textbf{182764.72} \\
paper 表 7 & 182726.8 & 182753.9 & 182761.9 & 182764.0 & \textbf{182764.7} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{逐值吻合到 $\pm0.2$ s，问题 4 的 Richardson 外推值吻合到 $\pm0.02$ s。}
这同时证明了两件事：(i) 本文对模型、参数、边界条件的理解与参考解完全一致；
(ii) 目标答案中问题 4 的 $182764.7$ s \textbf{确实是}「$\Delta t=10$ s 网格序列的二阶
Richardson 外推值」。既然模型一致，两者的连续极限必然相同，剩余差异只能是离散误差。

\subsection{第二步：定位误差来源——一阶时间误差}

paper 长时间段使用 $\Delta t=10$ s（表 7 中 $\Delta t=1$ s 的对照值比它大 13.6 s）。
补齐 $\Delta t$ 序列（$N=1600$）：
...
\textbf{典型的 $\mathcal{O}(\Delta t)$ 一阶误差}。用 $t(\Delta t)=L-c\,\Delta t$ 拟合得
$c\approx1.32$ s$\,/\,$s，故

\[
\text{问题 3: } L(N=1600,\Delta t\to0)=205568.8\ \mathrm{s},\qquad
\text{问题 4: } L(N=1600,\Delta t\to0)=182777.6\ \mathrm{s}.
\]

\subsection{第三步：空间收敛（全程 $\Delta t=1$ s）}
...
\textbf{与本文新解法的连续极限完全吻合。} 这构成一条闭合的证据链。
...
\caption{(a) 参考解格式的时间步收敛是一阶的（斜率 1 参考线），
其长时间段 $\Delta t=10$ s 引入约 13.6 s 系统偏差；
(b) 空间收敛对比：参考解格式（方格）为二阶，本文谱配置法（三角）
在 $N=48$ 就已落到 $10^{-2}$ s 以内。}
\label{fig:conv}
\end{figure}

目标答案中的 $t_{\text{dry}}$ 是参考解在\textbf{固定 $\Delta t=10$ s 的网格序列上做二阶
Richardson 外推}的结果。由于该序列被一阶时间误差主导，\textbf{外推并未收敛到连续极限}。
同一模型、同一参数下，真正收敛的答案是
\[
t_{\text{dry}}^{(3)}\approx205575.5\ \mathrm{s}\,(=57.1043\ \mathrm{h}),\qquad
t_{\text{dry}}^{(4)}\approx182778.3\ \mathrm{s}\,(=50.7717\ \mathrm{h}),
\]
与目标值相差 $0.007\sim0.008\%$；若按参考解自身引用的有效位数（$57.10$ h / $50.77$ h），
两者\textbf{完全一致}。
...
用 $D(C_0)=4.9377\times10^{-9}$ 反算，确认是 $e^{-0.89/C}$ \\
2 & 拟合 & 附件 1 的惯性拟合参数是否可信 &
用 Levenberg--Marquardt 独立复算：$50.2121/1877.49/0.050908/2776.34$，
RMSE $0.3359$ / $8.08\times10^{-4}$，与参考解一致 \\
3 & 口径 & 表面温度为何差 $8\times10^{-5}\,^\circ$C &
查到参考解程序使用\textbf{四舍五入}后的 $50.212/1877.5$；
改用后闭式解给出 $37.1989220\to37.1989$，与目标一致 \\
4 & 建模 & 收缩问题能否在 $r$ 坐标下直接写扩散方程 &
\textbf{不能}。该式仅在 $R$ 固定时成立；收缩时须补对流项
$-\dot Rr\partial_rC/R$（\S\ref{sec:shrink} 已给出证明与 $D=0$ 检验）\\
5 & 代码 & 参照有限体积末端节点的系数位置 &
...
再用独立有限体积复核到 $10^{-6}$ \\
10 & 结论 & $t_{\text{dry}}$ 差 16 s 是模型差异还是数值差异 &
原样复现参考解表 7 到 $\pm0.2$ s $\Rightarrow$ \textbf{模型一致}，
差异纯属其一阶时间误差 \\
\bottomrule
\end{longtable}
\end{center}
...
\item 本文提出的新解法\textbf{「坐标正则化 $u=(r/R)^2$ + Chebyshev 谱配置 +
自适应隐式 BDF + Bessel--Duhamel 闭式解」}，在空间离散、时间离散、
边界处理方式、问题 1 求解方式四条线上都与「节点有限体积 + 定步长
Crank--Nicolson + Richardson 外推」不同，且已被证明具有谱精度
（$N=8$ 达 $10^{-10}$）。

\item 关键 7 个答案中 \textbf{5 个与目标完全一致}（问题 1 三个、问题 2 三个之中心温度
与中心/表面含水），问题 1 的表 1 全部 35 个值完全一致。

\item 问题 3、4 的烘干时长与目标相差 $0.008\%$。经原样复现参考解格式并将其表 7
逐值重现，确认\textbf{模型完全一致}；差异来自参考解长时间段 $\Delta t=10$ s 的
一阶时间误差，其 Richardson 外推未收敛到连续极限。本文值
$205575.5$ s 与 $182778.3$ s 是经四种时间积分器、四个容差、谱空间加密一致确认的
连续极限。

\item 全部结果已按模板生成 \texttt{result1.xlsx}--\texttt{result4.xlsx}
（所有数值保留 4 位小数），并回读校验。
\end{enumerate}
