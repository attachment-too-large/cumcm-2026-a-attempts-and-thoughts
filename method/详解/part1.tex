...
径向的热流密度要乘以侧面积 $2\pi rL$ 才是总热流，所以算式里带一个 $r$。
这里的 $2\pi r\,\mathrm{d}r$ 是单位长度下的体积微元，不是面积。
侧面积是 $2\pi rL$，体积是 $2\pi rL\,\mathrm{d}r$。
把通量沿径向的净变化除以体积微元时，因子 $2\pi L$ 和 $\mathrm{d}r$ 都能约掉，
但是导数号里面的 $r$ 不能提到导数号外面去，所以分式前面留下一个 $1/r$。
这个 $1/r$ 就是式 \eqref{eq:lap} 里多出来的那一项。第二章会把这个过程完整写出来。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{fig/fig_cv.png}
\caption{圆柱环壳控制体与两种径向离散的对照。(a) 取半径 r 到 r+dr 的一层薄壳作控制体，侧面积是 2 pi r L；(b) 常规 r 坐标下的节点中心有限体积，第一个控制体只有 半格宽，其体积测度是 h 平方除以 8。}
\label{fig:cv}
\end{figure}

\subsection{能量守恒，一步不跳}

控制体是半径 $r$ 到 $r+\mathrm{d}r$、高度 $L$ 的环壳。
体积是 $2\pi r L\,\mathrm{d}r$，内能的变化率等于净流入的热量速率。
...
再算右边。规定 $q_r(r,t)$ 是沿正的 $r$ 方向的热流密度，
单位是瓦特每平方米。

在半径 $r$ 的柱面上，这个面是控制体的内表面。
热流沿正 $r$ 方向穿过它，就是从外面进入控制体，
所以单位时间从内表面进入的热量是正的 $q_r(r)$ 乘以面积 $2\pi rL$。

在半径 $r+\mathrm{d}r$ 的柱面上，热流沿正 $r$ 方向穿过它，
是从控制体流出去，
所以单位时间从外表面流出的热量是 $q_r(r+\mathrm{d}r)$
乘以面积 $2\pi(r+\mathrm{d}r)L$。

净流入等于流入减去流出：

\begin{equation}
\text{净流入} = q_r(r)\,2\pi r L - q_r(r+\mathrm{d}r)\,2\pi(r+\mathrm{d}r)L .
\label{eq:rhs}
\end{equation}

记 $G(r)=r\,q_r(r)$，则上式等于 $-2\pi L\left[G(r+\mathrm{d}r)-G(r)\right]$。
方括号里的量是一阶差分。
...
\begin{equation}
G(r+\mathrm{d}r) = G(r) + \frac{\partial G}{\partial r}\,\mathrm{d}r + \text{高阶项}.
\end{equation}

所以净流入等于 $-2\pi L\dfrac{\partial G}{\partial r}\mathrm{d}r$ 再加上高阶项。
这里用到的事实是：导数就是差商的极限，

\begin{equation}
\lim_{\mathrm{d}r\to0}\frac{G(r+\mathrm{d}r)-G(r)}{\mathrm{d}r}
= \frac{\partial G}{\partial r}.
\end{equation}

让 $\mathrm{d}r$ 趋于零，高阶项消失，得到

\begin{equation}
\text{净流入} = -2\pi L\,\frac{\partial}{\partial r}\left(r\,q_r\right)\mathrm{d}r .
\label{eq:rhs2}
\end{equation}

左边是内能变化率，也就是式 \eqref{eq:lhs2} 给出的量。两边相等，
再约去共同的 $2\pi L\,\mathrm{d}r$，得到

\begin{equation}
\rho c_p\,r\,\frac{\partial T}{\partial t}
= -\frac{\partial}{\partial r}\left(r\,q_r\right).
\label{eq:balance2}
\end{equation}

到这里负号还在。代入傅里叶定律 \eqref{eq:fourier}，
也就是 $q_r=-k\dfrac{\partial T}{\partial r}$：

\begin{equation}
-\frac{\partial}{\partial r}\left(r\left(-k\frac{\partial T}{\partial r}\right)\right)
= \frac{\partial}{\partial r}\left(r\,k\,\frac{\partial T}{\partial r}\right).
\end{equation}
...
\why{这里关于 $r$ 的运算，哪些可以做，哪些不可以做}
可以做的：两边同除以 $r$，因为 $r$ 大于零，
除完以后左边变成 $\rho c_p\dfrac{\partial T}{\partial t}$。
不可以做的：把导数号里面的 $r$ 提到导数号外面，

\begin{equation}
\frac{\partial}{\partial r}\left(r\,k\,\frac{\partial T}{\partial r}\right)
\ne r\,\frac{\partial}{\partial r}\left(k\,\frac{\partial T}{\partial r}\right).
\end{equation}

原因是这个 $r$ 随位置变化，不是常数因子，它要按乘积求导法则参与求导。
左边的 $r$ 来自体积公式里的圆周长，右边的 $r$ 在导数符号里面，
两者形式相同，来源不同。

\subsection{菲克定律与水分守恒}

水分的迁移和热量类似，也是由浓度差驱动，规律叫菲克定律。
按干基含水率写出来的通量形式是
...
其中 $D$ 是水分扩散系数，单位是平方米每秒，符号 $\mathrm{m^2/s}$，
$\rho_d$ 是单位湿体积内的干物质质量，下一小节会给出它的定义。
$j_r$ 是相对于干物质骨架的水分通量，单位是千克每平方米每秒，
也就是单位时间穿过单位面积的水的质量。
这里要带上 $\rho_d$，原因是干基含水率 $C$ 是一个比值，
单位是千克水每千克干物质，它本身不带质量密度的信息。
...
以干基含水率 $C$ 为变量，它的定义是每千克干物质里含有多少千克水。
在半径 $r$ 到 $r+\mathrm{d}r$ 的环壳里，干物质的质量是

\begin{equation}
\mathrm{d}m_d = \rho_d\,2\pi r L\,\mathrm{d}r ,
\end{equation}

其中 $\rho_d$ 是干物质在湿体积中的密度，它的定义在上一个小节里给出。
干物质不迁移，所以 $\mathrm{d}m_d$ 不随时间变化。
壳层里的水质量等于 $C$ 乘以 $\mathrm{d}m_d$。

规定 $j_r$ 是相对于干物质骨架的水分通量，
单位是每秒每平方米通过多少千克水。菲克定律给出
...
穿越半径 $r$ 的柱面进入壳层的水分速率是 $j_r(r)\,2\pi r L$，
穿越半径 $r+\mathrm{d}r$ 的柱面流出的速率是
$j_r(r+\mathrm{d}r)\,2\pi(r+\mathrm{d}r)L$。
与热量的情形完全相同，净流入等于流入减去流出，
记 $H(r)=r\,j_r(r)$，再做一次泰勒展开并取极限，得到

\begin{equation}
\text{净流入} = -2\pi L\,\frac{\partial}{\partial r}\left(r\,j_r\right)\mathrm{d}r .
\end{equation}

左边是水质量的变化率。因为 $\mathrm{d}m_d$ 是常数，
水的质量 $C\,\mathrm{d}m_d$ 的变化率等于 $\mathrm{d}m_d\dfrac{\partial C}{\partial t}$：

\begin{equation}
\rho_d\,2\pi r L\,\mathrm{d}r\,\frac{\partial C}{\partial t}
= -2\pi L\,\frac{\partial}{\partial r}\left(r\,j_r\right)\mathrm{d}r .
\end{equation}

约去 $2\pi L\,\mathrm{d}r$：

\begin{equation}
\rho_d\,r\,\frac{\partial C}{\partial t}
= -\frac{\partial}{\partial r}\left(r\,j_r\right).
\end{equation}

代入菲克定律 \eqref{eq:jC}，也就是
$j_r=-\rho_d D\dfrac{\partial C}{\partial r}$：

\begin{equation}
\rho_d\,r\,\frac{\partial C}{\partial t}
= \frac{\partial}{\partial r}\left(r\,\rho_d D\,\frac{\partial C}{\partial r}\right).
\end{equation}

两边同除以 $\rho_d$ 并同除以 $r$，得到
...
最后这一步把 $\rho_d$ 从导数号里面提出来约掉，
用到了干物质密度在同一个壳层内不随位置变化这个常用近似。
题目把 $D$ 定义为以干物质为基准的有效扩散系数，本文与题目保持同一口径。
...
所以题目里说的前 $30$ 分钟，也就是 $1800$ 秒，
温度明显在上升，表面含水率也已经在下降：
$1800$ 秒时表面含水率由 $2.55$ 降到 $1.5109$，降了约百分之四十一。
只有中心几乎没动。

% =====================================================================
\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{fig/fig_p1.png}
\caption{问题 1 的温度剖面与水分剖面演化。(a) 温度在 30 分钟内已经拉开明显梯度；(b) 水分在同样时间内几乎只动表层。两张图的差别来自热扩散率与水分扩散系数的量级差别。}
\label{fig:p1}
\end{figure}
...
轴对称的意思是：把整个圆柱绕轴旋转任意角度，温度分布不变。
也就是说，温度其实是横坐标 $x$ 的光滑函数，
其中 $x$ 是到轴的有符号距离，轴的一侧取正，另一侧取负。
中心线上 $x=0$，轴两侧对称的两个位置是 $x$ 和 $-x$，
它们的温度必须相同，所以这个函数是 $x$ 的偶函数。

偶函数在 $x=0$ 处的导数为零，也就是

\begin{equation}
\left.\frac{\mathrm{d}T}{\mathrm{d}x}\right|_{x=0}=0 .
\end{equation}

在 $r\ge0$ 的半轴上，$r$ 就是 $x$ 取正的那一半，
两者在 $x=0$ 附近的导数相同，所以在 $r=0$ 处温度对 $r$ 的一阶导数为零：
...
\why{计算时为什么把参数按有效位数取整}
上表里的拟合值是 $T_{\mathrm{set}}=50.2121$、$\tau_T=1877.49$、
$C_{\mathrm{set}}=0.050908$、$\tau_C=2776.34$。
实际计算用的是一组按有效位数报告的数：

\begin{equation}
T_{\mathrm{set}}=50.212,\qquad \tau_T=1877.5,\qquad
C_{\mathrm{set}}=0.050908,\qquad \tau_C=2776.3 .
\end{equation}

这四个数一起让表面温度变化约 $6\times10^{-5}$ 摄氏度。
其中设定值 $T_{\mathrm{set}}$ 的取整贡献 $4.14\times10^{-5}$ 摄氏度，
时间常数 $\tau_T$ 的取整贡献 $1.91\times10^{-5}$ 摄氏度，
两项相加就是 $6\times10^{-5}$ 摄氏度。
