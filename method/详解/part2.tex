...
它同时依赖含水率 $C$ 和温度 $T$。
这里要交代一个单位口径：经验式里温度那一项 $e^{-3850/T}$ 的 $T$
按附录规定用开尔文，
而全文其他地方出现的温度都用摄氏度。
所以程序里要先做一次换算 $T_{\mathrm{K}}=T+273.15$，
再把 $T_{\mathrm{K}}$ 代入这个经验式。
...
这类问题叫移动边界问题，处理起来需要额外的技巧，第九章详细讲。
...
常规的做法是对圆心附近做特殊处理。
记常规 $r$ 坐标下的节点间距为 $h$，节点排在 $r=0,h,2h,\dots$ 上。
那么最里面的那个控制体只覆盖 $r=0$ 到 $r=h/2$，
也就是半径 $h/2$ 的小圆柱，只有半格宽。
把它单独取出来，体积单独算，再单独写它对应的方程。
这样做当然可以，但会让程序变得复杂，
而且要额外推导公式，容易出错。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{fig/fig_map.png}
\caption{坐标变换 u 等于半径比值的平方。(a) 等距的半径映射成不等距的 u；(b) 同一批节点在两种坐标下的位置，上半行是 u 坐标，下半行是物理半径。}
\label{fig:map}
\end{figure}
...
第一项是 $D\dfrac{\partial\phi}{\partial u}$。要说明它在 $u=0$ 处有限，
不能直接断言，要按下面的链条推出来。

第一步，方程在 $u=0$ 处是正则的。式 \eqref{eq:reg} 的右边只出现 $u$ 的一次幂乘以导数，
没有 $1/u$，也没有别的奇性，所以 $u=0$ 与区间内部的点没有区别。

第二步，在这种方程下，解可以写成 $u$ 的收敛幂级数，也就是解在 $u$ 上是解析的。
第六章讨论谱方法时会把这个幂级数写出来。

第三步，幂级数可以逐项求导，所以解对 $u$ 的导数在 $u=0$ 处有界，
也就是存在一个有限的数 $M$，使得 $\left|\dfrac{\partial\phi}{\partial u}\right|\le M$。

第二项里有因子 $u$，在 $u=0$ 处直接等于零，
不需要知道 $\dfrac{\partial\phi}{\partial u}$ 在这一点的大小。

把这几条合起来：右边在 $u=0$ 处是一个有限的数，没有任何除以零的风险。
麻烦被彻底消掉了。
...
不需要。理由要用下面这条链条说，不能只看通量。

先说一个不能用的理由。当 $u=0$ 时，式 \eqref{eq:mass-u} 右边的通量
$uD\dfrac{\partial C}{\partial u}$ 前面有因子 $u$，等于零，
所以从圆心流出的总通量是零。
但这只是因为那里的面积是零，面积为零推不出
一阶导数 $\dfrac{\partial C}{\partial r}$ 为零。这两件事必须分开。

正确的理由链条有四步。

第一步，方程在 $u=0$ 处正则。式 \eqref{eq:reg} 的右边没有 $1/u$ 之类的奇性，
$u=0$ 与区间内部的点没有区别。

第二步，在这种方程下，解可以写成 $u$ 的收敛幂级数，也就是解在 $u$ 上解析。

第三步，幂级数逐项可导，所以解对 $u$ 的导数在 $u=0$ 处有界，
记这个界为 $M$，即 $\left|\dfrac{\partial C}{\partial u}\right|\le M$。

第四步，用链式法则回到 $r$。由式 \eqref{eq:drdx}，

\begin{equation}
\frac{\partial C}{\partial r}
= \frac{2\sqrt{u}}{R}\,\frac{\partial C}{\partial u}.
\end{equation}

当 $u$ 趋于零时，右边的因子 $\dfrac{2\sqrt{u}}{R}$ 趋于零，
而 $\dfrac{\partial C}{\partial u}$ 有界，所以整个右边趋于零。

于是 $r=0$ 处的一阶导数确实等于零，这正是对称条件。
它由方程本身和解的正则性保证，不需要另外写进程序。
...
\begin{equation}
\frac{\pi/N}{\pi^2/(2N^2)} = \frac{2N}{\pi} \approx 0.64N ,
\end{equation}

所以两端的点比中间密约 $\dfrac{2N}{\pi}$ 倍，也就是约 $0.64N$ 倍。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{fig/fig_cheb.png}
\caption{切比雪夫节点的来源。(a) 在单位圆上按等角距取点，再垂直投影到直径上；(b) 投影结果是两端密、中间疏的分布，两端间距约为 N 平方分之一量级。}
\label{fig:cheb}
\end{figure}
...
而当 $u=u_k$，也就是 $k$ 不等于 $j$ 时，分子里有一个因子 $u_k-u_k=0$，
所以 $\ell_j(u_k)=0$。
...
实际计算中，节点参数 $N$ 由 $4$ 加到 $6$，
与精确解的偏差就从 $7.59\times10^{-6}$ 掉到 $2.88\times10^{-10}$，
只多用了两个节点。误差不是慢慢下降，而是跳台阶，
这才是谱收敛的准确表现。

再往上加节点，偏差就停在 $10^{-10}$ 附近不再继续下降，
节点参数从 $8$ 加到 $64$ 都是这个水平。
这个平台不是空间离散的极限，而是时间积分的相对容差造成的地板：
节点数不变、只把时间相对容差收紧，偏差就跟着变小。
作收敛性检验时时间相对容差取 $10^{-12}$。

\keypoint
谱方法的三步是：选切比雪夫节点，构造微分矩阵，用矩阵乘法代替求导。
本题里节点参数 $N$ 从 $4$ 加到 $6$，
偏差就从 $7.59\times10^{-6}$ 掉到 $2.88\times10^{-10}$，
说明空间精度随节点数急剧提高。
偏差停在 $10^{-10}$ 是时间积分容差造成的地板，不是空间离散的极限。
...
而谱方法的节点在两端非常密集。
$u$ 坐标下最小的节点间距大约是 $\dfrac{\pi^2}{4N^2}$，
换成 $x$ 坐标就是 $\dfrac{\pi^2}{2N^2}$。
节点参数取 $64$ 时，实测的最小间距是 $6.023\times10^{-4}$，
公式给出 $6.024\times10^{-4}$，两者可以对上。

显式方法要求 $\Delta t$ 小于某个与最小节点间距的平方成正比的阈值。
这句话的意思是：节点越密，允许的步长越小，而且是按间距的平方缩小。
节点参数取 $64$ 时，实测的最小间距是 $6.023\times10^{-4}$，
它已经是网格中最小的一个，所以它决定步长上界。

现在算量级。先看物理过程本身的时间尺度。
时间方向的量纲因子是 $\dfrac{R^2}{4D}$，
取 $C=2.55$ 时的扩散系数，它约等于 $2.03\times10^4$ 秒。
也就是说，单靠扩散把水分走完两厘米，需要两万秒的量级。

再看显式方法允许的步长。
把同一套空间离散接到显式欧拉上实测：
步长取 $2\times10^{-4}$ 秒时稳定，取 $4\times10^{-4}$ 秒时发散，
所以稳定上界约为 $4\times10^{-4}$ 秒。

两个数一比就看出问题了：
允许的步长比物理过程的时间尺度小了约 $5\times10^7$ 倍。
也就是说每一秒要算约 $2500$ 步，
而整个过程要算 $2\times10^5$ 秒，总步数在 $5\times10^8$ 量级。
每步还要解一个含 $65$ 个未知量的方程组，
这个计算量是无法接受的。

这里只给出趋势与实测上界，不去套一个解析的步长公式。
原因是本题的右端项同时含温度与含水率，扩散系数本身随含水率变化，
最大特征值没有简单的解析表达式，老老实实测一次更可靠。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{fig/fig_p3.png}
\caption{问题 3 的干燥曲线。(a) 各半径处水分浓度随时间下降，横轴到 60 小时；(b) 终点附近放大，截面平均值先达标，中心最后达标，所以判据必须取全场最大值而不是平均值。}
\label{fig:p3}
\end{figure}
...
本题的实际数据是：截面面积加权平均含水率在 $35.9$ 小时就降到了 $0.15$，
而中心要到 $57.1$ 小时才达标，晚了将近 $21$ 小时。
用平均值判断会把时间低估约 $37\%$。

这里有一个必须交代的细节，否则上面这个数会被算错。
截面平均的定义是
...
其中权重 $r$ 来自环面积与半径成正比这一事实。
计算这个积分时，如果直接用谱方法节点上的值做加权平均，
结果会被拉低约 $0.2$，达标时刻提前十个多小时。
原因是切比雪夫节点在表面附近极密，而表面正是含水率最低的地方，
按节点直接平均等于给表层过高的权重。

正确的做法是先把解插值到均匀的半径网格，再按面积加权积分。
用交付的结果文件 result3.xlsx 复核时也要注意：
该文件只给出 $21$ 个半径列，用它做梯形积分得到 $35.2$ 小时，
把半径点数加到 $401$ 个得到 $35.8$ 小时，再加到 $2001$ 个仍是 $35.8$ 小时。
两者相差 $0.6$ 小时，差别来自积分点数，不是物理差异。
本文取收敛值 $35.9$ 小时。
...
导数怎么取近似 & 先由谱微分矩阵算出节点通量，再对通量整体求导 & 通量形式配置 \\
烘干时刻怎么确定 & 构造函数求零点 & 终端事件求交 \\
\bottomrule
\end{tabular}
\end{center}

表中的通量形式配置，做法是先用谱微分矩阵算出每个节点上的通量，
再对整个通量向量求一次导，它不是有限体积那种相邻控制体之间的通量差格式。
本文的对照格式才用有限体积。
...
那么每个材料点的含水率永远不会变，一直等于它自己的初始值。

再假设初始含水率不均匀，取中心为 $2.55$、表面为 $1.5$。
题目给的初值本来是处处相同的 $2.55$，
这里换成不均匀的分布，是为了让两种写法的差别显出来。

两个条件合起来得到什么结论？每个材料点的含水率都保持自己的初始值，
但是材料点在向内移动。本来在半径 $1.5$ 厘米处的干物质，
随着收缩慢慢移到半径 $1.0$ 厘米处，它带着自己的含水率一起走。

所以在固定的空间点 $r=1.0$ 厘米处，
一开始那里是内部的水，含水率高；后来移过来的是表层的水，含水率低。
含水率随时间变化，也就是
...
所以方程给出 $\left.\dfrac{\partial C}{\partial t}\right|_r=0$。

两个结果矛盾：物理推理给出变化，错误的方程给出不变。
矛盾的原因是，式 \eqref{eq:mass} 的推导假设了干物质不动，
它的时间导数是在固定空间位置求的。
收缩时干物质在移动，带着水分一起走，
所以固定空间点上的含水率会因为材料流过来而改变。
错误方程漏掉的正是这一部分变化。
...
本文没有给出偏差倍数的数值结论，误差的量级取决于收缩速率：
收缩越快、半径变化越大，漏掉的对流项越重要。
本文只用两条证据说明这一点。
一条是后面给出的解析验证，也就是把物质坐标下的解代回固定半径坐标的方程，
验证两种写法正好相差对流项。
另一条就是本节这个扩散系数为零的检验，
它说明固定半径坐标下的方程在含水率不均匀时给出的时间导数是错的。
...
注意 $j_r=-D\dfrac{\partial C}{\partial r}$ 的单位是米每秒。
原因是干基含水率 $C$ 是一个比值，单位是千克水每千克干物质，
它对 $r$ 求导以后的单位是每米，
再乘上单位是平方米每秒的 $D$，得到的就是米每秒。
这个量乘上干物质密度 $\rho_d$ 以后，单位才是千克每平方米每秒。

在物质坐标下做衡算时，用带上干物质密度的通量更方便，也就是

\begin{equation}
j_\xi = -\frac{\rho_d D}{R}\frac{\partial C}{\partial \xi},
\end{equation}

它等于 $j_r$ 乘上 $\rho_d$，单位是千克每平方米每秒，
含义是单位时间穿过单位面积的水的质量。

这里要说明一个记号上的差别。第一部分的式 \eqref{eq:jC} 里，
通量写成 $j_r=-\rho_d D\dfrac{\partial C}{\partial r}$，把 $\rho_d$ 含在里面，
单位是千克每平方米每秒。
本节为了做坐标换算方便，先把不带 $\rho_d$ 的那个通量记成 $j_r$，
它的单位是米每秒，两者相差一个 $\rho_d$ 因子。
...
其中 $\dot R$ 表示 $\mathrm{d}R/\mathrm{d}t$。
收缩时半径不断减小，所以 $\dot R$ 是负的；
收缩速率要取它的负值，也就是 $-\dot R$，这样才是正数。
